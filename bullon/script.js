...
function resetGame() {
  ...
  restartButton.style.display = "none";//一开始restbuuton是none的

  trees = [];
  for (let i = 1; i < window.innerWidth / 50; i++) generateTree();//生成一些树
...
function draw() {
  ...
  drawSky(); // Fill the background with a gradient画一个渐变的梯度背景作为天空

  ctx.save();/*save()主要用来保存目前Canvas的状态。通过save()函数它会将目前Canvas的状态推到绘图堆栈中。
